...
if($_SERVER['REQUEST_METHOD']=="POST" and(isset($_POST['submit2']))){
    ...
    {
        $cursor=$db->query("SELECT Status FROM tender WHERE T_id=$_POST[tid] and Pr_no=$_SESSION[pr]");
        ...
        $cursor=$db->query("UPDATE tender SET Status=$s WHERE T_id=$_POST[tid] and Pr_no=$_SESSION[pr]");
        //$val = $cursor->fetch_assoc();
        /*if(! $val ) {
            die('Could not enter data: ' . mysql_error());
        }else{
            echo "\n Status updated to $s, Entered data successfully\n";}*/
        }
    }
                        ...
                        <div class="main-box-body clearfix">
                            <div class="table-responsive">
                                <table border="0" cellpadding="10px" align="center" >
                                    <tr>
                                        <th align='center' style="padding: 15px; background: #DFDFDF;">   Pr No   </th>
                                        <th align='center' style="padding: 15px; background: #DFDFDF;">   Tender ID   </th>
                                        <th align='center' style="padding: 15px; background: #DFDFDF;">     Current Status    </th>
                                        <th align='center' style="padding: 15px; background: #DFDFDF;">   Update Status to   </th>
                                    </tr>
                                        ...
                                        <?php
                                            ...
                                            $db->select_db('proknap');
                                            $cursor = $db->query("SELECT tender.Pr_no,tender.T_id,tender.Status,status.Descr from tender join assign on tender.Pr_no=assign.Pr_no join status on tender.Status=status.Status_id WHERE assign.DO='$_SESSION[user_name]'");
                                            if ($db->error) {
                                            ...
                                            while($val = $cursor->fetch_assoc()){
                                                echo "<tr>";
                                                echo "<td align='center' style='padding: 15px;background: #FAFAFA;'>   ".$val['Pr_no']."   </td>";
                                                echo "<td align='center' style='padding: 15px;background: #FAFAFA;'>     ".$val['T_id']."   </td>";
                                                echo "<td align='center' style='padding: 15px;background: #FAFAFA;'>   ".$val['Descr']."   </td>";
                                                echo "<td align='center' style='padding: 15px;background: #FAFAFA;'>";
                                                if($val['Status']!=12 && $val['Status']!=5 && $val['Status']!=1){
                                                $cursor2 = $db->query("SELECT status.Descr from status WHERE Status_id=$val[Status]+1");
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>
                                                    <select name="tid">
                                                        ...
                                                        $db = new mysqli('localhost', 'root');
                                                            if(isset($_POST['submit3'])){
                                                                ...
                                                                $db->select_db('proknap');
                                                                $cursor = $db->query("SELECT T_id from tender where Status!=1 and Status!=12 and Status!=5 and Pr_no=$_POST[prid]");
                                                                if ($db->error) {
                                                                    echo $db->error.PHP_EOL;
                                                                }
                                                                //echo "<select>";
                                                    ...
                                                    <td><input name='submit2' type='submit' value="Update status"></input></td> ;
                                                </tr>
